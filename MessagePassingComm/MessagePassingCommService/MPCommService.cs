///////////////////////////////////////////////////////////////////////////////
///  MPCommService.cs - This package provides message passing communication
///                      using WCF.                                         ///
///  ver 3.0                                                                ///
///  Language:     C#                                                       ///
///  Platform:     windows10. toshiba satellite                             ///
///  Application:  Demonstrating Project 3 and To be used in project 4 of   ///
///                SMA CSE 681. client, mockRepository, motherBuilder       ///
 ...
 *   - postFile         : opens a channel and attempts to upload a file in blocks
 ...
 *   - closeCreatedChannel : closes the sender's factory channel. 
 ...
 * --------------------
 * var 3.0 : 29 Oct 2017
 * - added destinationAddress parameter to the postFile() of Comm class and Sender class.
 * - added closeCreatedChannel function in the comm class.
 * ver 2.0 : 19 Oct 2017
...
namespace MessagePassingComm
{
  ...
  public class Receiver : IMessagePassingComm
  {
    ...
    {  
      string address = baseAddress + ":" + port.ToString() + "/MessagePassingComm.Receiver";
    ...
    public void postMessage(CommMessage msg)
    {
      ...
      TestUtilities.putLine(string.Format("rcvr enqueuing message on thread {0}", Thread.CurrentThread.ManagedThreadId));
      rcvQ.enQ(msg);
    }
    ...
    public bool openFileForWrite(string name)
    {
      try
      {
        string writePath = Path.Combine(ServiceEnvironment.fileStorage, name);
                ...
                Console.WriteLine("\n writepath is {0}", Path.GetFullPath(writePath));
        fs = File.OpenWrite(writePath);
        return true;
      }
      catch(Exception ex)
      {
                Console.WriteLine("exception thron in openFileForWrite of receiver");
    ...
    public bool writeFileBlock(byte[] block)
    {
      ...
      {
                Console.WriteLine("exception thron in writefileblock of receiver");
                Console.WriteLine("details: {0}", ex.Message);
                Console.WriteLine("more and more details brother {0}", ex.StackTrace);
                Console.WriteLine(" ex.innerexception is: {0} \n ex.source is: {1}", ex.InnerException, ex.Source);
        lastError = ex.Message;
        return false;
      }
    }
  ...
  public class Sender
  {
    ...
    {
      port = listenPort;
      fromAddress = baseAddress + listenPort.ToString() + "/MessagePassingComm.Receiver";
    ...
    public bool postFile(string fileName, string destinationAddress)
    {
      ...
      try
      {
                if (lastUrl != destinationAddress)
                {
                    close();
                    lastUrl = destinationAddress;
                    createSendChannel(destinationAddress);
                }
                string path = Path.Combine(ClientEnvironment.fileStorage, fileName);
                Console.WriteLine("path of the file in sender is-- {0}", path);
        fs = File.OpenRead(path);
        bytesRemaining = fs.Length;
        bool canSend = channel.openFileForWrite(fileName);
       Console.Write("\n  File '{0}' was opened for read", fileName);
        while (true)
        {
      ...
      {
        lastError = ex.Message;
                Console.WriteLine("exception thrown in sender.postfile. details: {0}", ex.StackTrace);
        return false;
      }
      return true;
    }
